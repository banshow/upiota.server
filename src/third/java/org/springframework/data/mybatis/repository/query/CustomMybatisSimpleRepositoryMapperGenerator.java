...
public class CustomMybatisSimpleRepositoryMapperGenerator {
    ...
    public CustomMybatisSimpleRepositoryMapperGenerator(Configuration configuration, Dialect dialect, MybatisMappingContext context, Class<?> domainClass) {
    ...
    private String render() throws IOException {
        ...
        if (!isStatementExist("_insert")) {
            buildInsertSQL(builder, "_insert", false);
        }
        if (!isStatementExist("_insertIgnoreNull")) {
        	buildInsertSQL(builder, "_insertIgnoreNull", true);
        }
    ...
    private void buildInsertSQL(final StringBuilder builder,String statementName, final boolean ignoreNull) {
    	builder.append("<insert id=\""+statementName+"\" parameterType=\"" + domainClass.getName() + "\" lang=\"XML\"");
    	...
    	builder.append("insert into ").append(dialect.wrapTableName(persistentEntity.getTableName())).append("<trim prefix=\"(\" suffix=\")\" suffixOverrides=\",\" >");
    	...
    	builder.append("</trim> <trim prefix=\"values (\" suffix=\")\" suffixOverrides=\",\" >");
