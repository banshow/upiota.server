...
import org.springframework.data.repository.query.parser.CustomPartTree.OrPart;
import org.springframework.util.Assert;
import org.springframework.util.StringUtils;

public class CustomPartTree implements Iterable<OrPart> {
	/*
	 * We look for a pattern of: keyword followed by
	 *
	 * an upper-case letter that has a lower-case variant \p{Lu} OR any other
	 * letter NOT in the BASIC_LATIN Uni-code Block \\P{InBASIC_LATIN} (like
	 * Chinese, Korean, Japanese, etc.).
	 *
	 * @see <a
	 * href="http://www.regular-expressions.info/unicode.html">http://www.
	 * regular-expressions.info/unicode.html</a>
	 * 
	 * @see <a href=
	 * "http://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html#ubc"
	 * >Pattern</a>
	 */
	private static final String KEYWORD_TEMPLATE = "(%s)(?=(\\p{Lu}|\\P{InBASIC_LATIN}))";
	private static final String QUERY_PATTERN = "find|read|get|query|stream|select";
	...
	/**
	 * The subject, for example "findDistinctUserByNameOrderByAge" would have
	 * the subject "DistinctUser".
	 */
	 ...
	 * @param domainClass
	 *            the domain class to check individual parts against to ensure
	 *            they refer to a property of the class
	 */
	public CustomPartTree(String source, Class<?> domainClass) {
		...
		if (!matcher.find()) {
			...
			this.objective = new Objective(source, domainClass, false);
		} else {
			this.subject = new Subject(matcher.group(0));
			this.predicate = new Predicate(source.substring(matcher.group().length()), domainClass);
			boolean hasObjective = source.startsWith("select") && matcher.group(2) != null;
			
			this.objective = new Objective(hasObjective?matcher.group(2):source, domainClass, hasObjective);
		}
	}

	
	public boolean hasObjective() {
		return objective.hasObjective();
	}
	
	public Iterator<Part> objectiveIterator() {
		return objective.iterator();
	}
	...
	/**
	 * Returns the {@link Sort} specification parsed from the source or
	 * <tt>null</tt>.
	 * 
	 * @return the sort
	 */
	...
	/**
	 * return true if the created {@link PartTree} is meant to be used for
	 * delete operation.
	 * 
	 * @return
	 * @since 1.8
	...
	/**
	 * Return {@literal true} if the create {@link PartTree} is meant to be used
	 * for a query with limited maximal results.
	 * 
	 * @return
	 * @since 1.9
	 */
	...
	/**
	 * Splits the given text at the given keywords. Expects camel-case style to
	 ...
	 * around {@literal Or} keywords. Consists of {@link Part}s that have to be
	...
	public static class OrPart implements Iterable<Part> {
		 ...
		 * @param domainClass
		 *            the domain class to check the resulting {@link Part}s
		 *            against.
	...
	private static class Subject {
		...
		/**
		 * Returns {@literal true} if {@link Subject} matches
		 * {@link #DELETE_BY_TEMPLATE}.
		 * 
		 * @return
		 * @since 1.8
		 */
	...
	private static class Objective {
		...
		Objective(String source, Class<?> domainClass, boolean hasObjective) {
			...
			if (hasObjective) {
				...
				for (String part : split) {
					if (StringUtils.hasText(part)) {
						children.add(new Part(part, domainClass, true));
					}
				}
			}
