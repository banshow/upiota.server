...
public interface ResourceRepository extends MyMapper<Resource>{
	@Select("select rs.resource_key from up_resource rs left join up_role_resource rr on rs.id = rr.resource_id left join up_user_role ur on rr.role_id = ur.role_id where ur.user_id = #{userId}")
